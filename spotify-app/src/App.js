...
function App() {
    ...
    //Production const REDIRECT_URI = "https://yuantzhou.github.io/Spotify.github.io/"
    //local changes 
    const REDIRECT_URI = "http://localhost:3000/Spotify.github.io"
    const AUTH_ENDPOINT = "https://accounts.spotify.com/authorize"
    const RESPONSE_TYPE = "token"
    const scope = 'user-read-private user-read-email playlist-modify-public playlist-modify-private user-top-read user-library-read user-library-modify playlist-modify-private';
    ...
    const [numberOfTrim, setNumofTrim]= useState("");
    const [working, setWorkingPlayList] = useState([]);
    const [TrimedPlaylist,setTrimedPlaylist]= useState([]);
    // const getToken = () => {
    ...
    const renderNewFilterList = () => {
       ...
       if(tracksFeature.length>0 && fun== "Optimized"){
        ...
        return(<div className="generate"><button onClick={postNewSuggestionPlaylist} >Generate Playlist test changes</button></div>)
    }
...
const getTrim = async () => { 
    setTracksFeature([])
        ...
        setTrimedPlaylist([])
    for(let T of alltracks){
        //need to try mutipule artists for last.fm api call
        console.log(T.track)
        console.log(T.track.name)
        const data = await axios.get(`https://ws.audioscrobbler.com/2.0/?method=track.getInfo&username=yuanmusic35&api_key=1108fbe320adf2248c0a9ce523947018&artist=${encodeURIComponent(T.track.artists[0].name)}&track=${encodeURIComponent(T.track.name)}&format=json`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
        if(data.data.error){
            ...
            setWorkingPlayList(previous => [...previous,{name:T.track.name,id:T.track.id,addedAt:T.added_at,playcount:data.data.track.userplaycount}])
...
const Trimed =() =>{
    working.sort(function(a, b){return b.playcount-a.playcount});
    setTrimedPlaylist(working.slice(0,numberOfTrim))
}
...
const renderTrims=()=>{
    return(
        TrimedPlaylist.map(temp=> (
            <div key={temp.id} className="featureCell">
            <div className="pName">{temp.name}</div>
            <div className="pDetail">{temp.playcount}</div> 
            <div className="pDetail">{temp.addedAt}</div> 
            </div>
    ))
    )
...
const Joshstuff =  async() => { 
    ...
    const renderFun = ()=>{
        ...
        if(fun=="Trim"){return(<div><form  className="filterForm" onSubmit={Trimed}>
                        <p className="desHeader"> What is the Desirable PlayList size</p>
                            <p className="description">This will trim the less played songs in the play list to fit the Desirable size</p>
                        <p>Number of Songs</p><input type="number" onChange={e => setNumofTrim(e.target.value)} placeholder="Number of songs up to 30"/>
                        <button type={"submit"}>Filter</button>
                    </form>
                    <div className="featureCellHeader">
                        <div className="pNameHeader"> Song Name</div>
                        <div className="pDetailHeader"> Play Count</div>
                        <div className="pDetailHeader"> Date Added</div>          
                    </div>
                    </div>
                    )}
        ...
        <div className={!token? "":"container-fluid"}>
            <div className={!token? "":"row content"}>
                <div className="col-sm-3 sidenav">{!token ?
                    <a href={`${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&response_type=${RESPONSE_TYPE}&scopes=${scope}&show_dialog=true`} className="login">Login
                        to Spotify</a>
                ...
                {playlists&&token&&alltracks.length!==0? 
                    ...
                    <div className="box intro">
                        ...
                        <div className="btns">
                            ...
                            <button onClick={e => changeTrim(e.target.innerHTML)} className={fun=="Trim"? "btn btn-primary":"fun"}>Trim</button>
